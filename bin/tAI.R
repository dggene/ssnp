...
getws <- function(tRNA,  # tRNA gene copy number
...
tstest <- function(m, ws, nc, gc3s, ts.obs, samp.size=500, n=1000) {
  ...
  for(i in 1:length(ws.permuted[,1])) {
    tai <- gettai(m.samp, ws.permuted[i,])
